import numpy as np
from compute_histogram.main import _compute_histogram, add_histogram, _get_mask, _apply_mask
...
def test_get_mask():
    ...
    mask = _get_mask(a, 2)
    assert(np.all(mask == np.array([[False, False, True], [False, True, True], [True, True, True]])))

    mask = _get_mask(a, 2, mask_layer)
    assert(np.all(mask == np.array([[False, False, True], [False, False, True], [True, True, False]])))
